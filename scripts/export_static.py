"""匯出靜態資料到 docs/data/，供 GitHub Pages 託管。

資料來源仍是 data/stock.db，本腳本只是把查詢結果預先算好存成 JSON，
讓沒有伺服器的 GitHub Pages 也能呈現同樣的內容。
與後端 API 呼叫同一組 dataset 函式，因此兩種模式的資料格式一致。

只負責資料。網站本身的 HTML 與 JS 由前端建置產生，
再以 scripts/deploy_web.py 複製到 docs/，那一步只在改動前端時才需要，
因此 GitHub Actions 的每日更新不需要 Node.js。
